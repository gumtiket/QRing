#!/usr/bin/env bash
# server/를 빌드하고, 화이트리스트(dist/node_modules/public/package.json/drizzle)만 담아 zip으로
# 만든 뒤 S3 배포 버킷에 올린다. .env 등 server/ 밑의 다른 파일은 절대 포함하지 않는다.
# drizzle/은 부팅 시 dist/db/migrate.js가 읽는 마이그레이션 SQL이라 같이 올라간다.
set -euo pipefail
...
cp -r "$SERVER_DIR/drizzle" "$STAGING_DIR/drizzle"
...
echo "==> [4/5] zip 생성: dist, node_modules, public, package.json, drizzle 만"
(cd "$STAGING_DIR" && zip -r -q "$ZIP_PATH" dist node_modules public package.json drizzle)
